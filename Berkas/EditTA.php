...
<div class="page-heading">
    ...
    $(".btn-primary").click(function(event) {
        ...
        var formData = new FormData($("form")[0]); // Get all form data
        formData.append("action", "editTA"); // Add the action field with value "editTA"
        
        var nim = $("input[name='NIM']").val(); // Get NIM value
        
        // Confirm if the user wants to edit
        if (confirm("Apakah Anda yakin ingin mengedit data ini?")) {
            $.ajax({
                url: "ProsesBerkas.php", // Your server-side script
                type: "POST",
                data: formData,
